...
class SubTwoCategoryController extends AdminController
{
    ...
    {
        $products = Product::where('sub_category_id',$id)->where('status',1)->where('deleted',0)->where('publish','Y')->get()->count();
        $users = User::where('active',1)->get();
        // if($products > 0){
        //     session()->flash('danger', trans('messages.can_not_add_cat'));
        //     return back();
        // }else{
        return view('admin.categories.sub_category.sub_two_category.create',compact('id', 'users'));
        // }
    }
